...
const LevelService = {
    ...
    getRankTitle(level) {
        ...
        const subRank = Math.max(1, 10 - Math.floor((offset / levelSpan) * 10));

        return `${rank.code}${subRank} ${rank.title}`;
    },
...
const TaskService = {
    ...
    completeTask(currentState, taskId, remark = "") {
        ...
        const pointsAwarded = Math.max(
            1,
            Math.floor(Number(task.rewardPoints) || DEFAULT_TASK_REWARD_POINTS)
        ) + (bonusAwarded ? WEEKLY_TARGET_BONUS_POINTS : 0);
...
function handleTaskActions(event) {
    ...
    if (!actionButton || actionButton.dataset.action === "complete") {
        return;
    }
...
function handleReset() {
    ...
    state.points = freshState.points;
    state.rewards = freshState.rewards;
    state.redemptions = freshState.redemptions;
...
function renderHero() {
    ...
    elements.nextRank.textContent = nextRank
        ? `Next rank: ${LevelService.getRankTitle(nextRank.min)} at level ${nextRank.min}`
        : `Final rank: Absolute Monarch at level ${MAX_LEVEL}`;
...
function renderTaskCard(task, isCurrentDaySelected, todayKey) {
    ...
    if (isCurrentDaySelected && completedToday) {
        ...
        statusCopy = task.repeatType === "weekly"
            ? `Worth 10 XP and ${formatPoints(rewardPoints)}, with a 20 XP and ${formatPoints(WEEKLY_TARGET_BONUS_POINTS)} bonus when the weekly target is reached.`
            : `Worth 10 XP and ${formatPoints(rewardPoints)} on completion.`;
    }
...
function sanitizeRedemptions(redemptions) {
    ...
    return redemptions.reduce((result, rawRedemption) => {
        if (!rawRedemption || typeof rawRedemption !== "object") {
            return result;
        }
