...
const Storage = {
    ...
    loadSyncEvents() {
        return sanitizeSyncEvents(this.loadJSON(STORAGE_KEYS.syncEvents, []));
    },
    ...
    saveSyncEvents(events) {
        localStorage.setItem(STORAGE_KEYS.syncEvents, JSON.stringify(sanitizeSyncEvents(events)));
    },
...
const SyncService = {
    ...
    async pushLatestState(reason = "update") {
        ...
        try {
            ...
            Storage.saveState(state);
            const eventsToSync = pendingSyncEvents.slice();
            ...
            if (eventsToSync.length) {
                pendingSyncEvents = pendingSyncEvents.slice(eventsToSync.length);
                Storage.saveSyncEvents(pendingSyncEvents);
            }
            Storage.saveState(state);
            renderApp();
            this.setStatus("synced", "Sync: live");
...
let pendingSyncEvents = Storage.loadSyncEvents();
...
function handleCompletionSubmit(event) {
    ...
    queueSyncEvent({
        ...
        details: {
            ...
            leveledUp: result.levelInfo.leveledUp,
            levelsGained: result.levelInfo.levelsGained,
            newLevel: result.levelInfo.newLevel,
            reachedMax: result.levelInfo.reachedMax
        }
    });
    persistAndRender();
...
function handleReset() {
    ...
    queueSyncEvent({
        ...
        taskName: "",
        category: "",
        remark: "All Rankion data reset by user.",
        xpDelta: 0,
        details: {}
    });
    persistAndRender();
...
function sanitizeSyncEvents(events) {
    ...
    return events
        .map((event) => sanitizeSyncEvent(event))
        .filter(Boolean)
        .slice(-200);
}
...
function sanitizeSyncEvent(event) {
    ...
    const eventType = String(event.eventType || "").replace(/\s+/g, "_").trim().slice(0, 60);
    if (!eventType) {
        return null;
    }
    ...
    return {
        timestamp: normalizeCompletedAt(event.timestamp || new Date().toISOString()),
        eventType,
        taskName: normalizeTaskName(event.taskName).slice(0, 80),
        category: CATEGORY_DEFINITIONS.some((item) => item.key === category) ? category : "",
        remark: normalizeRemark(event.remark),
        xpDelta: Math.max(0, Math.floor(Number(event.xpDelta) || 0)),
        currentLevel: Math.min(MAX_LEVEL, Math.max(1, Math.floor(Number(event.currentLevel) || state.level || 1))),
        currentXp: Math.max(0, Math.floor(Number(event.currentXp) || state.xp || 0)),
        currentStreak: Math.max(0, Math.floor(Number(event.currentStreak) || 0)),
        details: event.details && typeof event.details === "object" ? event.details : {}
    };
}

function queueSyncEvent(eventInput) {
    const event = sanitizeSyncEvent({
        ...eventInput,
    ...
    pendingSyncEvents = sanitizeSyncEvents([...pendingSyncEvents, event]);
    Storage.saveSyncEvents(pendingSyncEvents);
}
